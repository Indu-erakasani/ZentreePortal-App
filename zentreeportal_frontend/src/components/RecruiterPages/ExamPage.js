...
function useProctoring(examToken) {
  ...
  // ── capture — NOW ALSO SAVES FRAME TO BACKEND ─────────────────────────────
  const capture = useCallback(async (label = "periodic") => {
    ...
    if (sum / (data.length / 4 * 3) < 5) return null;
    ...
    try {
      // analyze + store frame in one call
      const res = await fetch(`${BASE}/exams/proctor/${examToken}/analyze`, {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ image: b64, label, ts }),  // ts added
      });
      const d  = await res.json();
      const ai = (d.success && d.data) ? d.data : { flag: "ok", reason: "" };
  ...
  }, [ready, examToken, addEvent]);

  // ── audio monitoring — unchanged ───────────────────────────────────────────
        ...
        if (avg > 20) {
          ...
          if (voiceCount === 6) {
            addEvent({
              ...
              type:     "warning",
              msg:      "Voice/audio detected — talking during exam",
              snapshot: null,
            });
            voiceCount = 0;
          }
  ...
  const startVideoRecording = useCallback((stream) => {
    try {
      ...
      if (!mimeType) {
        console.warn("[Proctor] No supported video format for recording");
        return;
      }
      ...
      // ── STORE THE UPLOAD PROMISE SO doSubmit CAN AWAIT IT ──
      recorder.onstop = () => {
        const blob = new Blob(videoChunksRef.current, { type: "video/webm" });
        console.log(`[Proctor] Uploading video: ${(blob.size / 1024 / 1024).toFixed(2)} MB`);

        if (blob.size < 1000) {
          console.warn("[Proctor] Video blob too small, skipping upload");
          videoUploadPromiseRef.current = Promise.resolve();
          return;
        }
        ...
        videoUploadPromiseRef.current = fetch(
          `${BASE}/exams/proctor/${examToken}/upload-video`,
          {
            method: "POST",
            body: formData,
            // NO Authorization header — upload route is public (uses token in URL)
          }
        )
          .then(r => r.json())
          .then(d => console.log("[Proctor] Video uploaded:", d))
          .catch(e => console.warn("[Proctor] Video upload failed:", e));
      };

      recorder.start(10000);
      mediaRecorderRef.current = recorder;
      console.log("[Proctor] Video recording started");

    } catch (e) {
      console.warn("[Proctor] MediaRecorder failed:", e);
    }
  }, [examToken]);

  // ── start — pass existing stream from CameraGate ──────────────────────────
  ...
  }, [startAudioMonitoring, startVideoRecording]);

  // ── periodic capture — unchanged ──────────────────────────────────────────
  useEffect(() => {
    if (!ready) return;
    const t = setTimeout(() => capture("initial"), 4000);
    timerRef.current = setInterval(() => capture("periodic"), 8000);
    ...
    const fn = () => {
      if (document.hidden) {
        addEvent({
          ...
          ts:       new Date().toISOString(),
          type:     "alert",
          msg:      "Tab switch / window blur detected",
          snapshot: null,
        });
      }
    };
...
const stop = useCallback(() => {
  ...
  // Stop recorder — triggers onstop → sets videoUploadPromiseRef
  if (mediaRecorderRef.current && mediaRecorderRef.current.state !== "inactive") {
    mediaRecorderRef.current.stop();
  }

  // ── DON'T stop stream tracks here — stop them after upload ──
  setReady(false);
}, []);

// ── NEW: waitForUpload — call this before submitting exam ──
const waitForUpload = useCallback(async () => {
  streamRef.current?.getTracks().forEach(t => t.stop());

  // Wait up to 3s for onstop to fire and set the promise
  let waited = 0;
  while (!videoUploadPromiseRef.current && waited < 3000) {
...
return { videoRef, ready, error, events, snaps, faceAlert, start, stop, waitForUpload, capture };
}
...
function CameraGate({ exam, onGranted }) {
  ...
  const requestCamera = async () => {
    ...
    try {
      ...
      setStep("verifying");
      // wait 2 s for video to stabilise before capturing
  ...
  // ── capture one frame and ask the backend to verify it ────────────────────
  const captureAndVerify = async (stream) => {
    ...
    try {
      ...
      // ── calls backend verify-face route (no token needed here) ──
      const res = await fetch(`${BASE}/exams/proctor/verify-face`, {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ image: b64 }),
      });
      const d  = await res.json();
      // backend returns { success, data: { approved, reason, ... } }
      const ai = (d.success && d.data) ? d.data : { approved: false, reason: "Verification failed" };

      if (ai.approved) {
        // hand the already-open stream to the parent — no second getUserMedia call
        onGranted(stream);
      } else {
      ...
      // on backend failure, fail open so the exam isn't blocked
      onGranted(stream);
    }
  };
...
function RightSidebar({ total, currentFlat, statusMap, onJump, videoRef, camReady, camError, events }) {
  ...
  const voiceCount = events.filter(e => e.msg?.includes("Voice") || e.msg?.includes("audio")).length;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* KEY FIX: always render the video element so the ref is stable */}
        <video
          ref={videoRef}
          autoPlay
          muted
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            ...
            {voiceCount > 0 && (
              <div style={{ background: "#6a1b9a", borderRadius: 8, padding: "1px 6px", fontSize: 9, color: "#fff", fontWeight: 700 }}>
                {voiceCount}🎤
              </div>
            )}
          </div>
        ...
        {tab === "scores" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              <div style={{
                flex: 1, minWidth: 130,
                background: integrityScore >= 80 ? "#e8f5e9" : integrityScore >= 60 ? "#fff3e0" : "#fce4ec",
                borderRadius: 12, padding: 16, textAlign: "center",
                border: `1px solid ${integrityScore >= 80 ? "#a5d6a7" : integrityScore >= 60 ? "#ffcc02" : "#f48fb1"}`,
              }}>
...
export default function ExamPage() {
  ...
  // ── camera granted — reuse the stream the gate already opened ─────────────
  const handleCameraGranted = useCallback(async (stream) => {
    setCameraGranted(true);
    await proctor.start(stream);   // injects existing stream — no second getUserMedia
  }, [proctor]);
  ...
  const doSubmit = useCallback(async (auto = false) => {
    ...
    // ── Stop recorder first, then wait for video upload ──
    proctor.stop();
    setSubmitting(true);
  
    // Give recorder ~500ms to fire onstop and start the upload fetch
    // await new Promise(resolve => setTimeout(resolve, 500));
  
    // Wait for video upload to complete (max 15 seconds)
    const uploadTimeout = new Promise(resolve => setTimeout(resolve, 15000));
    await Promise.race([proctor.waitForUpload(), uploadTimeout]);
    ...
    allQ.forEach((e, i) => {
      ...
      if (e.type === "coding")     codeList.push({
        question_index: e.origIdx, code: val,
        run_output: compRes[i]?.stdout  || "",
        run_stderr: compRes[i]?.stderr  || "",
        run_status: compRes[i]?.status  || "",
      });
    });
  ...
  // ── keep a stable ref to doSubmit so the timer never captures a stale copy ─
  ...
  useEffect(() => {
    ...
    if (timeLeft === null || submitted) return;
    if (timeLeft <= 0) { doSubmitRef.current(true); return; }
    const t = setTimeout(() => setTimeLeft(s => s - 1), 1000);
    return () => clearTimeout(t);
  }, [timeLeft, submitted, cameraGranted]);   // doSubmit intentionally omitted — use ref above
  ...
  const goNext = useCallback(() => {
    ...
    if (inReview) {
      ...
      if (nextPos < reviewQueue.length) {
        ...
        if (stillSkipped.length > 0) { setReviewQueue(stillSkipped); setReviewPos(0); }
    ...
    if (next < totalQ) {
      ...
      if (skippedList.length > 0) { setReviewQueue(skippedList); setReviewPos(0); setInReview(true); }
    }
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 28, height: 28, background: "#fff", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontWeight: 900, fontSize: 14, color: "#1a237e" }}>Z</span>
          </div>
          <span style={{ color: "#fff", fontWeight: 800, fontSize: 15 }}>ZentreeLabs<sup style={{ fontSize: 8 }}>PRO</sup></span>
        </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <span style={{ background: meta.color, color: "#fff", borderRadius: 6, padding: "3px 12px", fontWeight: 700, fontSize: 13 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 11px", borderRadius: 20, background: meta.color + "18", color: meta.color }}>
                {meta.label}
              </span>
            </div>
                      ...
                      <div style={{ background: "#13131f" }}>
                        ...
                        {cr.stdout && (
                          <div style={{ padding: "8px 14px" }}>
                            <pre style={{ margin: 0, fontSize: 12, color: "#a6e22e", fontFamily: "monospace", whiteSpace: "pre-wrap" }}>{cr.stdout}</pre>
                          </div>
                        )}
                        {(cr.stderr || cr.compile_output) && (
                          <div style={{ padding: "8px 14px" }}>
                            <pre style={{ margin: 0, fontSize: 12, color: "#f92672", fontFamily: "monospace", whiteSpace: "pre-wrap" }}>{cr.compile_output || cr.stderr}</pre>
                          </div>
                        )}
                      </div>
                    );
          ...
          <div style={{
            ...
            {showSubmit ? (
              ...
              <button onClick={goNext} style={{
                ...
                color: "#fff", fontWeight: 700, fontSize: 13,
                cursor: "pointer", boxShadow: `0 3px 12px ${meta.color}44`,
              }}>Save & Next →</button>
            )}
          </div>
        </div>

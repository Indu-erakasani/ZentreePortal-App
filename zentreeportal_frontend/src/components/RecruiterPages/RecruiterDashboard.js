...
//   Alert, LinearProgress, Avatar, List, ListItem, ListItemAvatar,
//   ListItemText, Divider, Button, Table, TableHead, TableBody,
//   TableRow, TableCell, Paper, IconButton, Tooltip, Badge,
// } from "@mui/material";
// import {
//   People, Work, CheckCircle, TrendingUp, AccessTime,
//   Star, Add, MonetizationOn, Business, WorkOutline,
...
// const STAGE_COLORS = {
//   "Screening":         "#546e7a",
//   "Technical Round 1": "#0277bd",
//   "Technical Round 2": "#01579b",
//   "HR Round":          "#e65100",
//   "Manager Round":     "#6a1b9a",
//   "Final Round":       "#ad1457",
//   "Offer Stage":       "#1565c0",
//   "Negotiation":       "#f9a825",
...
//             textTransform="uppercase" letterSpacing={0.6}>{title}</Typography>
//           <Typography variant="h3" fontWeight={800} mt={0.5} sx={{ color }}>{value ?? "—"}</Typography>
//           {sub && <Typography fontSize={12} color="text.secondary" mt={0.4}>{sub}</Typography>}
//         </Box>
//         <Avatar sx={{ bgcolor: `${color}18`, color, width: 48, height: 48 }}>{icon}</Avatar>
//       </Box>
//       {progress !== undefined && (
//         <LinearProgress variant="determinate" value={Math.min(progress, 100)}
//           sx={{ height: 4, borderRadius: 3, bgcolor: `${color}18`,
//             "& .MuiLinearProgress-bar": { bgcolor: color, borderRadius: 3 } }} />
//       )}
//     </CardContent>
//   </Card>
...
// // ── Mini Calendar ─────────────────────────────────────────────────────────────
// function MiniCalendar({ allInterviews, onDaySelect, selectedDay, calYear, calMonth,
...
//       <Button size="small" startIcon={<Today fontSize="small" />} onClick={onToday}
//         fullWidth variant="outlined" sx={{ mb: 1.5, textTransform: "none", fontSize: 11,
//           borderRadius: 2 }}>
//         Today
//       </Button>
...
//           if (!day) return <Box key={i} />;
//           const hasEvents  = !!dayMap[day];
//           const isToday    = today.getDate() === day && today.getMonth() === calMonth
//                              && today.getFullYear() === calYear;
//           const isSelected = selectedDay === day;
//           const count      = dayMap[day]?.length || 0;

//           return (
//             <Box key={i} onClick={() => hasEvents && onDaySelect(day)}
//               sx={{
//                 aspectRatio: "1", display: "flex", flexDirection: "column",
//                 alignItems: "center", justifyContent: "center", borderRadius: 1.5,
//                 cursor: hasEvents ? "pointer" : "default",
//                 bgcolor: isSelected ? "#1a237e" : isToday ? "#e3f2fd" : "transparent",
//                 border: isToday && !isSelected ? "1.5px solid #90caf9" : "1.5px solid transparent",
//                 transition: "all 0.15s",
//                 "&:hover": hasEvents ? {
//                   bgcolor: isSelected ? "#1a237e" : "#f0f4ff"
//                 } : {},
//                 position: "relative",
//               }}>
...
//               {hasEvents && (
//                 <Box sx={{
//                   width: count > 1 ? 14 : 6, height: 6,
//                   borderRadius: 3,
//                   bgcolor: isSelected ? "#fff" : "#0277bd",
//                   mt: 0.3, fontSize: 8,
//                   display: "flex", alignItems: "center", justifyContent: "center",
//                 }}>
//                   {count > 1 && (
//                     <Typography fontSize={8} color={isSelected ? "#1a237e" : "#fff"}
//                       fontWeight={700} lineHeight={1}>{count}</Typography>
//                   )}
//                 </Box>
//               )}
...
//       {/* Legend */}
//       <Box mt={2} pt={1.5} borderTop="1px solid #f0f0f0">
//         <Box display="flex" alignItems="center" gap={1}>
//           <Box sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: "#0277bd" }} />
//           <Typography fontSize={10} color="text.secondary">Has interviews</Typography>
//         </Box>
//       </Box>
//     </Box>
//   );
...
//       // Auto-select today if it has interviews
//       const todayStr = new Date().toDateString();
//       const todayHas = (upcoming.data || []).some(ev =>
//         new Date(ev.scheduled_at).toDateString() === todayStr
//       );
//       if (todayHas) setSelectedDay(new Date().getDate());
...
//       }}>
//         <CardContent sx={{ p: 2.5, display: "flex", justifyContent: "space-between",
//           alignItems: "center", flexWrap: "wrap", gap: 2 }}>
//           <Box>
//             <Typography variant="h5" fontWeight={800}>
//               Welcome back{myData?.recruiter_name
//                 ? `, ${myData.recruiter_name.split(" ")[0]}` : ""}! 👋
//             </Typography>
//             <Typography sx={{ opacity: 0.85, mt: 0.5, fontSize: 14 }}>
//               {kpis.open_jobs ?? 0} open positions ·
//               {kpis.total_candidates ?? 0} candidates ·
//               {kpis.placements_mtd ?? 0} placements this month
//               {todayCount > 0 && (
//                 <Chip label={`${todayCount} interview${todayCount > 1 ? "s" : ""} today`}
//                   size="small"
//                   sx={{ ml: 1.5, bgcolor: "#fff3e0", color: "#e65100",
...
//               <Grid item xs={6} md={3} key={label}>
//                 <Box display="flex" alignItems="center" gap={1.5}
//                   sx={{ borderRight: i < 3 ? { md: "1px solid #f0f0f0" } : "none",
//                     pr: { md: 2 } }}>
//                   <Avatar sx={{ bgcolor: `${color}12`, color, width: 38, height: 38 }}>{icon}</Avatar>
//                   <Box>
//                     <Typography fontWeight={800} fontSize="1.1rem" color={color}>{value}</Typography>
//                     <Typography fontSize={11} color="text.secondary">{label}</Typography>
//                   </Box>
//                 </Box>
//               </Grid>
...
//       {/* ══════════════════════════════════════════════════════════════════════
//           MAIN ROW: Calendar (left) + Pipeline + Candidates (right)
//       ══════════════════════════════════════════════════════════════════════ */}
//       <Grid container spacing={2.5}>
...
//               {/* Calendar header */}
//               <Box display="flex" alignItems="center" gap={1} mb={2}>
//                 <CalendarMonth sx={{ color: "#0277bd", fontSize: 20 }} />
//                 <Typography fontWeight={700} fontSize={15}>Interview Calendar</Typography>
//                 {todayCount > 0 && (
//                   <Chip label={`${todayCount} today`} size="small"
//                     sx={{ ml: "auto", bgcolor: "#e3f2fd", color: "#0277bd",
//                           fontWeight: 700, fontSize: 10 }} />
//                 )}
//               </Box>
...
//                   textTransform="uppercase" letterSpacing={0.5} mb={1.2}>
//                   {selectedDay
//                     ? `${selectedDay} ${MONTHS[calMonth]} — ${selectedDayInterviews.length} interview${selectedDayInterviews.length !== 1 ? "s" : ""}`
//                     : `${MONTHS[calMonth]} — ${selectedDayInterviews.length} scheduled`}
//                 </Typography>

//                 {selectedDayInterviews.length === 0 ? (
//                   <Box textAlign="center" py={2}>
//                     <CalendarMonth sx={{ fontSize: 32, color: "#e0e0e0", mb: 0.5 }} />
...
//                           bgcolor: isNow ? "#e8f5e9" : "#f8f9fa",
//                           border: isNow ? "1px solid #a5d6a7" : "1px solid #f0f0f0",
//                           transition: "all 0.15s",
//                         }}>
//                           <Box display="flex" alignItems="center" gap={1}>
//                             <Box sx={{
//                               width: 3, height: 36, borderRadius: 2, flexShrink: 0,
//                               bgcolor: STAGE_COLORS[ev.current_stage] || "#0277bd",
//                             }} />
//                             <Box flex={1} minWidth={0}>
...
//                                   href={ev.meeting_link} target="_blank"
//                                   component="a"
//                                   sx={{ color: "#34a853", bgcolor: "#e8f5e9",
...
//                 <Box display="flex" alignItems="center" gap={1} mb={2}>
//                   <TrendingUp sx={{ color: "#0277bd" }} />
//                   <Typography fontWeight={700} fontSize={15}>Active Pipeline</Typography>
//                   <Chip
//                     label={`${pipelineDisplay.reduce((s, p) => s + p.count, 0)} total`}
//                     size="small" sx={{ ml: "auto", fontSize: 11 }} />
//                 </Box>
...
//                 {pipelineDisplay.length === 0 ? (
//                   <Box display="flex" alignItems="center" justifyContent="center" py={3} gap={1}>
//                     <PersonOff sx={{ color: "#bdbdbd" }} />
//                     <Typography fontSize={13} color="text.disabled">No active candidates</Typography>
//                   </Box>
//                 ) : (
...
//                               sx={{ maxWidth: "80%" }}>{stage}</Typography>
//                             <Typography fontSize={11} fontWeight={700}
//                               color={STAGE_COLORS[stage] || "#546e7a"}>{count}</Typography>
//                           </Box>
//                           <LinearProgress variant="determinate"
//                             value={(count / maxPipelineCount) * 100}
//                             sx={{
//                               height: 5, borderRadius: 4,
//                               bgcolor: `${STAGE_COLORS[stage] || "#546e7a"}15`,
//                               "& .MuiLinearProgress-bar": {
//                                 bgcolor: STAGE_COLORS[stage] || "#546e7a", borderRadius: 4,
//                               },
//                             }} />
//                         </Box>
//                       </Grid>
...
//                     border: "1px solid #bbdefb", display: "flex", alignItems: "center",
//                     justifyContent: "space-between" }}>
//                     <Box display="flex" alignItems="center" gap={1}>
//                       <Star sx={{ color: "#f57f17", fontSize: 16 }} />
//                       <Typography fontWeight={700} fontSize={12}>Fill Rate</Typography>
//                     </Box>
//                     <Typography variant="h5" fontWeight={800} color="#0277bd">
//                       {kpis.fill_rate}%
//                     </Typography>
//                   </Box>
//                 )}
...
//                   <Typography fontWeight={700} fontSize={15}>My Recent Candidates</Typography>
//                   <Button component={Link} to="/resumes" size="small"
...
//                           <Avatar sx={{ bgcolor: "#1a237e", fontWeight: 700,
//                             fontSize: 12, width: 32, height: 32 }}>
//                             {nameInitials(c.name)}
//                           </Avatar>
//                           <Box flex={1} minWidth={0}>
//                             <Typography fontWeight={600} fontSize={13} noWrap>{c.name}</Typography>
...
//                             sx={{
//                               fontSize: 10, fontWeight: 700, flexShrink: 0,
//                               bgcolor: `${STAGE_COLORS[c.status] || "#546e7a"}15`,
//                               color: STAGE_COLORS[c.status] || "#546e7a",
//                             }} />
//                         </Box>
//                         {i < Math.min(myCandidates.length, 5) - 1 && (
...
//           <CardContent sx={{ p: 2.5 }}>
//             <Box display="flex" justifyContent="space-between" alignItems="center" mb={1.5}>
//               <Typography fontWeight={700} fontSize={15}>High Priority Jobs</Typography>
//               <Button component={Link} to="/jobs" size="small"
//                 sx={{ fontSize: 11, textTransform: "none" }}>View All →</Button>
//             </Box>
//             <Paper variant="outlined" sx={{ borderRadius: 2, overflow: "hidden" }}>
//               <Table size="small">
//                 <TableHead>
...
//       {/* ── Bottom: Recruiter Performance + Recent Activity ── */}
//       <Grid container spacing={2.5}>
//         <Grid item xs={12} md={7}>
//           <Card sx={{ borderRadius: 3 }}>
//             <CardContent sx={{ p: 2.5 }}>
...
//                               <LinearProgress variant="determinate"
//                                 value={Math.min(r.conversion_rate || 0, 100)}
//                                 sx={{ width: 44, height: 4, borderRadius: 3,
//                                   "& .MuiLinearProgress-bar": {
//                                     bgcolor: (r.conversion_rate || 0) >= 30 ? "#2e7d32" : "#0277bd"
//                                   }}} />
//                               <Typography fontSize={10} color="text.secondary">
...
//           <Card sx={{ borderRadius: 3, height: "100%" }}>
//             <CardContent sx={{ p: 2.5 }}>
//               <Typography fontWeight={700} fontSize={15} mb={1.5}>Recent Activity</Typography>
//               {recentActivity.length === 0 ? (
//                 <Box display="flex" alignItems="center" justifyContent="center" py={4} gap={1}>
//                   <AccessTime sx={{ color: "#bdbdbd" }} />
//                   <Typography fontSize={13} color="text.disabled">No recent activity</Typography>
//                 </Box>
//               ) : (
...
//                         </Avatar>
//                         <Box flex={1} minWidth={0}>
//                           <Typography fontSize={12} color="text.primary" lineHeight={1.4}>
//                             {a.message}
//                           </Typography>
...
//         <Card sx={{ borderRadius: 3 }}>
//           <CardContent sx={{ p: 2.5 }}>
//             <Typography fontWeight={700} fontSize={15} mb={2}>Client Revenue Overview</Typography>
//             <Grid container spacing={2}>
//               {clientRevenue.map((c, i) => {
//                 const maxRev = Math.max(...clientRevenue.map(x => x.revenue), 1);
//                 const colors = ["#1a237e","#0277bd","#2e7d32","#e65100","#7b1fa2","#00838f"];
...
//                 return (
//                   <Grid item xs={12} sm={6} md={4} key={i}>
//                     <Box p={2} bgcolor="#f8fbff" borderRadius={2} border="1px solid #e3f2fd">
//                       <Box display="flex" alignItems="center" gap={1} mb={0.8}>
//                         <Business sx={{ fontSize: 15, color }} />
//                         <Typography fontSize={13} fontWeight={700} noWrap>{c.client}</Typography>
//                       </Box>
//                       <Typography variant="h5" fontWeight={800} color={color}>
//                         {fmt(c.revenue)}
...
//                       <LinearProgress variant="determinate"
//                         value={(c.revenue / maxRev) * 100}
//                         sx={{ mt: 1.2, height: 4, borderRadius: 3,
//                           bgcolor: `${color}15`,
//                           "& .MuiLinearProgress-bar": { bgcolor: color, borderRadius: 3 }}} />
//                     </Box>
//                   </Grid>

...
import {
  ...
  VideoCall, AccessTime, FiberManualRecord, ArrowUpward,
  Business, PersonSearch, Assignment,
...
const STATUS_META = {
  ...
  Recruiter_Accepted:       { bg: "#eff6ff", color: "#0369a1",  label: "Recruiter Accepted" },
  ...
  Recruiter_Rejected:       { bg: "#fef2f2", color: "#dc2626",  label: "Recruiter Rejected" },
  HiringManager_Rejected:   { bg: "#fef2f2", color: "#991b1b",  label: "HM Rejected" },
  Recruiter_Hold:           { bg: "#fff7ed", color: "#c2410c",  label: "On Hold" },
  OnHold_TestPassed:        { bg: "#fff7ed", color: "#c2410c",  label: "Hold (Test Passed)" },
  HiringManager_Hold:       { bg: "#fff7ed", color: "#c2410c",  label: "HM Hold" },
  Interview_Scheduled:      { bg: "#eff6ff", color: "#1d4ed8",  label: "Interview Scheduled" },
  Round2_Scheduled:         { bg: "#eff6ff", color: "#1d4ed8",  label: "Round 2 Scheduled" },
...
export default function RbotRecruiterDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [rbotRes, calRes, jdsRes] = await Promise.allSettled([
        authFetch(`${BASE}/rbot-dashboard/recruiter`),
  ...
  const jdBreakdown  = (data?.jd_breakdown || []).filter(jd =>
    !search ||
  ...
  // Categorise statuses for the breakdown
        ...
        <Grid item xs={12} md={4}>
          <Card elevation={0} sx={{ borderRadius: "16px", border: "1px solid #e2e8f0", height: "100%" }}>
            <CardContent sx={{ p: 2.5 }}>
              ...
              {/* Positive / Negative / Hold summary */}
                    ...
                    <TableHead>
                      <TableRow sx={{ bgcolor: "#f8fafc" }}>
                        {["Candidate", "Status", "Match", "Test Score",
                          "Recruiter Feedback", "HM Feedback"].map(h => (

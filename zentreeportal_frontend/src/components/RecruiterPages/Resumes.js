...
const getAllResumes = (p = {}) => {
  ...
  return fetch(`${RESUMES_BASE}/${qs ? "?" + qs : ""}`, { headers: getHeaders() }).then(handle);
};
const createResume = (payload) =>
  fetch(`${RESUMES_BASE}/`, { method: "POST", headers: getHeaders(), body: JSON.stringify(payload) }).then(handle);
const updateResume = (id, payload) =>
  fetch(`${RESUMES_BASE}/${id}`, { method: "PUT", headers: getHeaders(), body: JSON.stringify(payload) }).then(handle);
const deleteResume = (id) =>
  fetch(`${RESUMES_BASE}/${id}`, { method: "DELETE", headers: getHeaders() }).then(handle);
const getAllJobs = () =>
...
const getRawResumes = (p = {}) => {
  const qs = new URLSearchParams(p).toString();
  return fetch(`${RESUMES_BASE}/raw/${qs ? "?" + qs : ""}`, { headers: getHeaders() }).then(handle);
};
const uploadRawResume = (file_b64, file_name) =>
  fetch(`${RESUMES_BASE}/raw/upload`, {
    method: "POST", headers: getHeaders(),
    body: JSON.stringify({ file_b64, file_name }),
  }).then(handle);
const assignRawToJob = (id, payload) =>
  fetch(`${RESUMES_BASE}/raw/${id}/assign-job`, {
    method: "PUT", headers: getHeaders(),
    body: JSON.stringify(payload),
  }).then(handle);
const convertRaw = (id, payload) =>
  fetch(`${RESUMES_BASE}/raw/${id}/convert`, {
    method: "POST", headers: getHeaders(),
    body: JSON.stringify(payload),
  }).then(handle);
const deleteRaw = (id) =>
  fetch(`${RESUMES_BASE}/raw/${id}`, { method: "DELETE", headers: getHeaders() }).then(handle);
const createRawManual = (payload) =>
  fetch(`${RESUMES_BASE}/raw/manual`, {
    method: "POST", headers: getHeaders(),
    body: JSON.stringify(payload),
  }).then(handle);
...
const createTrackingEntry = (payload) =>
  fetch(`${TRACKING_BASE}/`, { method: "POST", headers: getHeaders(), body: JSON.stringify(payload) }).then(handle);
...
const PdfViewerDialog = ({ open, onClose, candidate }) => {
  ...
  React.useEffect(() => {
    ...
    fetch(`${RESUMES_BASE}/${candidate._id}/file`, { headers: getHeaders() })
        ...
        <form onSubmit={handleConvert}>
          <DialogContent sx={{ pt: 3 }}>
            ...
            <Grid container spacing={2} mb={2}>
              ...
              <Grid item xs={12} sm={4}><TextField fullWidth size="small" type="number" label="Experience (yrs)" name="experience" value={convertData.experience} onChange={handleConvertChange} inputProps={{ min: 0 ,step: 0.01}} /></Grid>
                ...
                {currentEntry.saved ? (
                  ...
                  <>
                    <Grid container spacing={2} mb={2}>
                      ...
                      <Grid item xs={12} sm={4}><TextField sx={{ width: "100%", minWidth: 400 }} size="small" type="number" label="Experience (yrs)" name="experience" value={currentEntry.formData.experience} onChange={handleBulkChange} inputProps={{ min: 0,step: 0.01 }} /></Grid>
        ...
        <form onSubmit={handleSave}>
          <DialogContent sx={{ pt: 3 }}>
            ...
            <Grid container spacing={2} mb={2}>
              ...
              <Grid item xs={12} sm={6}><TextField sx={{ width: "100%", minWidth: 400 }} size="small" type="number" label="Experience (years)" name="experience" value={formData.experience} onChange={handleChange} inputProps={{ min: 0 ,step: 0.01}} /></Grid>
        ...
        <form onSubmit={handleManualRawSave}>
          <DialogContent sx={{ pt: 3 }}>
            ...
            <Grid container spacing={2} mb={2}>
              ...
              <Grid item xs={12} sm={4}><TextField fullWidth size="small" type="number" label="Experience (yrs)" name="experience" value={manualRawData.experience} onChange={handleManualRawChange} inputProps={{ min: 0,step: 0.01 }} /></Grid>

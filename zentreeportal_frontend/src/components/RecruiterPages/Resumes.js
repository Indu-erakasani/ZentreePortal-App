...
//   Close as CloseIcon, PictureAsPdf, OpenInNew,
...
// // Resume file URL — token in query param so browser can open it in new tab
// // Upload a PDF for an already-created candidate (used by Add Candidate form)
...
// // ── PDF Viewer Dialog — full-screen iframe ────────────────────────────────────
// // ── PDF Viewer Dialog — fetches PDF as blob with auth header ─────────────────
// // Avoids the "Missing Authorization Header" error that occurs when an iframe
// // tries to load a URL directly (browsers cannot set headers on iframe src).
...
//     // Clean up blob URL when dialog closes or candidate changes
//     return () => {
...
//         {!fetching && fetchErr && (
//           <Box display="flex" flexDirection="column" alignItems="center"
//             justifyContent="center" height="100%" gap={2} p={3}>
...
//             <Typography fontSize={13} color="text.disabled">Upload a PDF via the drag-and-drop zone</Typography>
//           </Box>
//         )}

//         {/* PDF iframe — uses blob URL so auth header is respected */}
//         {!fetching && blobUrl && (
//           <iframe
//             src={blobUrl}
...
//   const [addFile,    setAddFile]    = useState(null);   // File object selected in Add Candidate
...
//       (expBand.min === "10" ? r.experience >= 10
//         : r.experience >= Number(expBand.min) && r.experience <= Number(expBand.max));
//     return mQ && mS && mJ && mE;
//   });
...
//         // Upload replacement file if one was selected during edit
//         if (addFile) {
//           const b64 = await toBase64(addFile);
//           await uploadFileForCandidate(selected._id, b64).catch(() => {});
//         }
//       } else {
//         const created = await createResume(payload);
//         // Upload the PDF immediately after creating the candidate record
...
//       file: f, status:"pending", file_id:"",
//       formData: { ...EMPTY_FORM, status:"New" },
//       saved: false, errorMsg:"",
//     }));
//     setInlineFiles(entries);
//     setShowParsing(true);
...
//         const file_id = result.file_id || "";   // ← backend returns file_id after saving PDF
//         updated[idx] = {
//           ...updated[idx], status:"done", file_id,
//           formData: {
//             ...EMPTY_FORM, ...parsed,
...
//         };
//       } catch (err) {
//         const file_id = err?.file_id || "";    // backend saves file even on 422 parse error
//         updated[idx] = {
//           ...updated[idx], status:"error", file_id,
...
//         file_id: entry.file_id || "",   // ← backend will move temp PDF to permanent location
//       });
...
//     } catch (err) {
//       setBulkFiles(prev => prev.map((e, i) =>
//         i === bulkStep ? { ...e, errorMsg: err?.message || "Save failed" } : e
//       ));
...
//   const allParsed    = inlineFiles.length > 0 && inlineFiles.every(f => f.status !== "parsing" && f.status !== "pending");
...
//             onClick={() => inlineRef.current?.click()} size="large">
//             Upload Resume
//           </Button>
...
//       {/* ── Inline upload zone (always visible) ── */}
//       {!showParsing ? (
...
//           <TextField
//             placeholder="Search by name, skills, or ID…" value={search}
...
//               Drop PDF resumes in the upload zone above or add a candidate manually.
//             </Typography>
...
//                     <TableCell>
//                       <Box display="flex" alignItems="center" gap={1.5}>
//                         <Avatar sx={{ width:36, height:36, fontSize:13, fontWeight:700, bgcolor:"#1a237e" }}>
//                           {nameInitials(r.name)}
//                         </Avatar>
//                         <Box>
...
//                             sx={{ fontSize:10, height:20, bgcolor:"#e8eaf6", color:"#1a237e" }} />
//                         ))}
...
//                     <TableCell sx={{ fontSize:12, color:"#0277bd" }}>{r.linked_job_title||"—"}</TableCell>
//                     <TableCell>
...
//                           </IconButton>
//                         </Tooltip>
//                         {/* PDF button — red when file exists, grey/disabled when not */}
//                         <Tooltip title={r.resume_file ? "View Resume PDF" : "No resume file uploaded"}>
//                           <span>
//                             <IconButton size="small"
//                               onClick={() => r.resume_file && openPdf(r)}
...
//       {/* ══════ BULK REVIEW DIALOG ════════════════════════════════════════════ */}
//       <Dialog open={bulkOpen} onClose={closeBulk} maxWidth="md" fullWidth
...
//                 AI-extracted details pre-filled · PDFs already saved · review and confirm each
//               </Typography>
//             </Box>
//           </Box>
//         </DialogTitle>
...
//                     <Chip key={i}
//                       label={`${i+1}. ${entry.file.name.replace(".pdf","").slice(0,22)}`}
//                       onClick={() => setBulkStep(i)}
//                       color={entry.saved ? "success" : i === bulkStep ? "primary" : "default"}
//                       variant={i === bulkStep ? "filled" : "outlined"}
//                       size="small"
...
//                     {bulkStep < bulkFiles.length - 1 && (
//                       <Button variant="outlined" onClick={() => setBulkStep(s => s + 1)}>Next Resume →</Button>
//                     )}
//                   </Box>
//                 ) : (
//                   <>
...
//                         <TextField select fullWidth size="small" label="Source" sx={{ width: "100%", minWidth: 200 }}name="source" value={currentEntry.formData.source} onChange={handleBulkChange}>
//                           {SOURCES.map(s => <MenuItem key={s} value={s}>{s}</MenuItem>)}
//                         </TextField>
//                       </Grid>
//                       <Grid item xs={12} sm={4}>
//                         <TextField select fullWidth size="small" label="Status" sx={{ width: "100%", minWidth: 200 }}name="status" value={currentEntry.formData.status} onChange={handleBulkChange}>
//                           {STATUSES.map(s => <MenuItem key={s} value={s}>{s}</MenuItem>)}
//                         </TextField>
//                       </Grid>
//                         <TextField fullWidth multiline rows={2} sx={{ mt:2 }} label="Skills (comma-separated)" name="skills" value={currentEntry.formData.skills} onChange={handleBulkChange} placeholder="e.g. React, Node.js, MongoDB" /></Grid>
//                     <Divider sx={{ my:2 }} />
...
//                       <Grid item xs={12} sm={4}>
//                         <TextField select fullWidth size="small"sx={{ width: "100%", minWidth: 200 }} label="Notice Period" name="notice_period" value={currentEntry.formData.notice_period} onChange={handleBulkChange}>
//                           {NOTICES.map(n => <MenuItem key={n} value={n}>{n}</MenuItem>)}
//                         </TextField>
//                       </Grid>
//                       <Grid item xs={12}>
//                         <TextField select fullWidth size="small" label="Link to Job"sx={{ width: "100%", minWidth: 200 }} name="linked_job_id" value={currentEntry.formData.linked_job_id} onChange={handleBulkChange}>
...
//               <Grid item xs={12} sm={6}><TextField fullWidth size="small" required type="email" label="Email" name="email" value={formData.email} onChange={handleChange} disabled={!!selected} /></Grid>
...
//                 <TextField select fullWidth size="small" label="Source"sx={{ width: "100%", minWidth: 200 }} name="source" value={formData.source} onChange={handleChange}>
//                   {SOURCES.map(s => <MenuItem key={s} value={s}>{s}</MenuItem>)}
//                 </TextField>
//               </Grid>
//               <Grid item xs={12}><TextField fullWidth size="small" label="Skills (comma-separated)" name="skills" value={formData.skills} onChange={handleChange} placeholder="e.g. React, Node.js, MongoDB" /></Grid>
//             </Grid>
//             <Divider sx={{ my:2 }} />
//             <Typography variant="subtitle2" color="primary" mb={1.5} fontWeight={700}>Compensation &amp; Availability</Typography>
//             <Grid container spacing={2} mb={2}>
...
//               <Grid item xs={12} sm={4}>
//                 <TextField select fullWidth size="small" sx={{ width: "100%", minWidth: 200 }} label="Notice Period" name="notice_period" value={formData.notice_period} onChange={handleChange}>
//                   {NOTICES.map(n => <MenuItem key={n} value={n}>{n}</MenuItem>)}
//                 </TextField>
//               </Grid>
//               <Grid item xs={12} sm={6}>
//                 <TextField select fullWidth size="small" label="Status"sx={{ width: "100%", minWidth: 200 }} name="status" value={formData.status} onChange={handleChange}>
//                   {STATUSES.map(s => <MenuItem key={s} value={s}>{s}</MenuItem>)}
//                 </TextField>
//               </Grid>
//               <Grid item xs={12} sm={6}>
//                 <TextField select fullWidth sx={{ width: "100%", minWidth: 200 }}size="small" label="Linked Job" name="linked_job_id" value={formData.linked_job_id} onChange={handleChange}>
//                   <MenuItem value="">No Job Linked</MenuItem>
//                   {jobs.map(j => <MenuItem key={j._id} value={j._id}>{j.title}</MenuItem>)}
//                 </TextField>
//               </Grid>
...
//             {/* ── PDF upload for Add / Edit Candidate ── */}
//             <Divider sx={{ my:2 }} />
...
//               sx={{
//                 border: addFile ? "2px solid #2e7d32" : "2px dashed #90caf9",
//                 borderRadius: 2,
//                 p: 2,
//                 display: "flex",
//                 alignItems: "center",
...
//                     <Typography fontSize={11} color="text.secondary">
//                       {(addFile.size / 1024).toFixed(0)} KB · Click to replace
//                     </Typography>
//                   </>
//                 ) : (
//                   <>
...
//                         ? `Current file: ${selected.resume_file} · click to replace`
...
//                 <Typography color="text.secondary">{selected.current_role} at {selected.current_company}</Typography>
//                 <Chip label={selected.status} color={STATUS_COLOR[selected.status]||"default"} size="small" sx={{ mt:0.5, fontWeight:700 }} />
//               </Box>
//             </Box>
...
//                 ["Current Salary",  fmtSalary(selected.current_salary)],
//                 ["Expected Salary", fmtSalary(selected.expected_salary)],
//                 ["Notice",          selected.notice_period||"—"],
...
//                   {selected.skills.split(",").filter(Boolean).map((s,i) => (
...
//                 <Typography fontWeight={700} color="primary.dark">{selected.linked_job_title}</Typography>
//               </Box>
//             )}

//             {/* ── Resume PDF section ── */}
//             <Box p={2} borderRadius={2} display="flex" alignItems="center" gap={2}
//               sx={{ bgcolor: selected.resume_file ? "#e8eaf6" : "#f5f5f5", border:"1px solid", borderColor: selected.resume_file ? "#e8eaf6" : "#e0e0e0" }}>
//               <PictureAsPdf sx={{ fontSize:36, color: selected.resume_file ? "#1a237e" : "#bdbdbd", flexShrink:0 }} />
//               <Box flex={1}>
//                 <Typography fontWeight={700} fontSize={13} color={selected.resume_file ? "success.dark" : "text.secondary"}>
//                   {selected.resume_file ? "Original Resume PDF" : "No resume file uploaded"}
//                 </Typography>
//                 <Typography fontSize={11} color="text.secondary">
//                   {selected.resume_file
//                     ? `Stored as ${selected.resume_file} · click to view`
...
//                   onClick={() => { setDetailOpen(false); openPdf(selected); }}
//                 >
//                   View PDF
//                 </Button>
//               )}
...
const InlineUploadZone = ({ onFiles, fileRef }) => {
  ...
  return (
    ...
    >
      ...
      {/* <Button variant="outlined" size="small" startIcon={<CloudUpload fontSize="small" />}
...
export default function Resumes() {
  ...
  const [expF,       setExpF]       = useState("");
  const [jobF,       setJobF]       = useState("");
  const [clientF,    setClientF]    = useState("");   // filter by client
  ...
  // All job IDs that belong to the selected client (used for client filter)
  const clientJobIds = clientF
    ? jobs.filter(j => j.client_id === clientF).map(j => j._id)
    : null;
  ...
  const filtered = resumes.filter(r => {
    ...
    const mE = !expBand || expBand.label === "All Experience" ||
      (expBand.min === "10" ? r.experience >= 10
        : r.experience >= Number(expBand.min) && r.experience <= Number(expBand.max));
    // Client filter: resume must be linked to a job belonging to the selected client
    ...
    <Box display="flex" flexDirection="column" gap={3}>
      ...
      {resumes.length > 0 && (
        <Box display="flex" gap={2} flexWrap="wrap">
          ...
          {/* ── Client filter ── */}
          <TextField select value={clientF} onChange={e => { setClientF(e.target.value); setJobF(""); }}
          ...
          {/* ── Job filter — scoped to selected client ── */}
          <TextField select value={jobF} onChange={e => setJobF(e.target.value)}
            size="small" sx={{ minWidth:180 }} label="Job">
            <MenuItem value="">All Jobs</MenuItem>
            {(clientF
              ? jobs.filter(j => j.client_id === clientF)
              : jobs
            ).map(j => <MenuItem key={j._id} value={j._id}>{j.title}</MenuItem>)}
          </TextField>
        </Box>
      )}
                ...
                {filtered.length === 0 ? (
                  ...
                  <TableRow key={r._id} hover>
                    ...
                    <TableCell>
                      {r.linked_job_title ? (
                        <Box>
                          {/* Look up client name from the linked job */}
                          {(() => {
        ...
        <form onSubmit={handleSave}>
          <DialogContent sx={{ pt:3 }}>
            ...
            <Grid container spacing={2} mb={2}>
              ...
              <Grid item xs={12} sm={6}><TextField fullWidth size="small" type="number" label="Experience (years)" name="experience" value={formData.experience} onChange={handleChange} inputProps={{ min:0 }} /></Grid>
              <Grid item xs={12} sm={6}>
                <TextField select fullWidth size="small" sx={{ width: "100%", minWidth: 200 }} label="Source" name="source" value={formData.source} onChange={handleChange}>
                  {SOURCES.map(s => <MenuItem key={s} value={s}>{s}</MenuItem>)}
                </TextField>
              </Grid>
              {/* <Grid item xs={12}> */}
                <TextField fullWidth multiline rows={2} sx={{ mt:2 }} label="Skills (comma-separated)" name="skills" value={formData.skills} onChange={handleChange} placeholder="e.g. React, Node.js, MongoDB" /></Grid>
            {/* </Grid> */}
            <Divider sx={{ my:2 }} />
            <Typography variant="subtitle2" color="primary" mb={1.5} fontWeight={700}>Compensation &amp; Availability</Typography>
            <Grid container spacing={2} mb={2}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField select fullWidth size="small"  sx={{ width: "100%", minWidth: 200 }}label="Status" name="status" value={formData.status} onChange={handleChange}>
                  {STATUSES.map(s => <MenuItem key={s} value={s}>{s}</MenuItem>)}
                </TextField>
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField select fullWidth size="small"  sx={{ width: "100%", minWidth: 200 }} label="Linked Job" name="linked_job_id" value={formData.linked_job_id} onChange={handleChange}>
                  <MenuItem value="">No Job Linked</MenuItem>
                  {jobs.map(j => <MenuItem key={j._id} value={j._id}>{j.title}</MenuItem>)}
                </TextField>
              </Grid>
      ...
      <Dialog open={detailOpen} onClose={() => setDetailOpen(false)} maxWidth="sm" fullWidth>
        ...
        {selected && (
          <DialogContent sx={{ pt:3 }}>
            ...
            {/* ── Resume PDF section ── */}
            <Box p={2} borderRadius={2} display="flex" alignItems="center" gap={2}
              sx={{ bgcolor: selected.resume_file ? "#f5f5f5" : "#f5f5f5", border:"1px solid", borderColor: selected.resume_file ? "#e0e0e0" : "#e0e0e0" }}>
              {/* <PictureAsPdf sx={{ fontSize:36, color: selected.resume_file ? "#bdbdbd" : "#bdbdbd", flexShrink:0 }} /> */}
              <Box flex={1}>
                <Typography fontWeight={700} fontSize={13} color={selected.resume_file ? "success.dark" : "text.secondary"}>
                  {selected.resume_file ? "Original Resume PDF" : "No resume file uploaded"}
                </Typography>

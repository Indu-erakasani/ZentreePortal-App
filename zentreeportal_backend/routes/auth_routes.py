...
def register():
    ...
    # ── Duplicate check in local DB ───────────────────────────────────────────
    if User.find_by_email(email):
        return jsonify({"success": False, "message": "Email is already registered"}), 409

    # ── NEW: For recruiters, also check ResourcingBot DB ─────────────────────
    if role == "recruiter" and User.find_in_resourcingbot_by_email(email):
        return jsonify({
            "success": False,
            "message": "Email already exists in ResourcingBot. Please login instead."
        }), 409

    try:
        # User.create() handles dual-write to ResourcingBot internally for recruiters
        user = User.create(first_name, last_name, email, password, role, phone=phone)
        return jsonify({
            "success": True,
            "message": "Account created successfully",
...
def login():
    ...
    # ── Step 1: Try local ZentreePortal DB (existing logic) ──────────────────
    user = User.find_by_email(email)
    ...
    # ── Step 2: NEW — Fallback to ResourcingBot DB (recruiters only) ─────────
    rb_user = User.find_approved_in_resourcingbot(email)

    if not rb_user:
        # Not found in either DB
        return jsonify({"success": False, "message": "Invalid email or password"}), 401

    if not User.check_password(password, rb_user["password"]):
        return jsonify({"success": False, "message": "Invalid email or password"}), 401

    # ── Auto-provision into ZentreePortal on first cross-DB login ─────────────
    # Splits ResourcingBot's single "name" field into first/last
    name_parts = rb_user.get("name", "").split(" ", 1)
    try:
        provisioned = User.create(
            first_name=name_parts[0],
            last_name=name_parts[1] if len(name_parts) > 1 else "",
            email=rb_user["email"],
            password=password,              # plain — create() hashes it
            role="recruiter",
            phone=rb_user.get("phone"),
        )
    except Exception:
        return jsonify({"success": False, "message": "Account sync failed. Contact admin."}), 500

    user_id = str(provisioned["_id"])
    User.update_last_login(user_id)
    access_token, refresh_token = generate_tokens(user_id, "recruiter")

    return jsonify({
        "success": True,
        "message": "Login successful. Account synced to ZentreePortal.",
        "access_token": access_token,
        "refresh_token": refresh_token,
        "user": User.serialize(provisioned),
        "first_time_sync": True             # optional flag — useful for frontend onboarding
    }), 200
...
def change_password():
    ...
    # ── NEW: update_password() handles dual-write to ResourcingBot internally ─
